...
test() ->
    ...
    etap:fun_is(
        ...
        cbt_file:open(filename() ++ ".1", [create, invalid_option]),
        "Invalid flags to open are ignored."
    ),

    {ok, Fd} = cbt_file:open(filename() ++ ".0", [create, overwrite]),
    etap:ok(is_pid(Fd),
    ...
    ?etap_match(cbt_file:append_term(Fd, foo), {ok, 0, _},
        "Appending a term returns the previous end of file position."),

    {ok, Size} = cbt_file:bytes(Fd),
    etap:is_greater(Size, 0,
        "Writing a term increased the file size."),

    ?etap_match(cbt_file:append_binary(Fd, <<"fancy!">>), {ok, Size, _},
        "Appending a binary returns the current file size."),

    etap:is({ok, foo}, cbt_file:pread_term(Fd, 0),
        "Reading the first term returns what we wrote: foo"),

    etap:is({ok, <<"fancy!">>}, cbt_file:pread_binary(Fd, Size),
        "Reading back the binary returns what we wrote: <<\"fancy\">>."),

    etap:is({ok, cbt_compress:compress(foo, snappy)},
        cbt_file:pread_binary(Fd, 0),
        "Reading a binary at a term position returns the term as binary."
    ),

    {ok, BinPos, _} = cbt_file:append_binary(Fd, <<131,100,0,3,102,111,111>>),
    etap:is({ok, foo}, cbt_file:pread_term(Fd, BinPos),
        "Reading a term from a written binary term representation succeeds."),

    BigBin = list_to_binary(lists:duplicate(100000, 0)),
    {ok, BigBinPos, _} = cbt_file:append_binary(Fd, BigBin),
    ...
    etap:is({ok, BigBin}, cbt_file:pread_binary(Fd, BigBinPos2),
        "Reading a large term from a written representation succeeds 2."),

    % append_binary == append_iolist?
    % Possible bug in pread_iolist or iolist() -> append_binary
    {ok, IOLPos, _} = cbt_file:append_binary(Fd, ["foo", $m, <<"bam">>]),
    {ok, IoList} = cbt_file:pread_iolist(Fd, IOLPos),
    ...
    etap:is(ok, cbt_file:sync(Fd),
        "Syncing does not cause an error."),

    etap:is(ok, cbt_file:truncate(Fd, Size),
        "Truncating a file succeeds."),

    %etap:is(eof, (catch cbt_file:pread_binary(Fd, Size)),
