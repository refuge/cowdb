...
-type cowdb_stream() :: pid().
-type cowdb_stream_options() :: [encoding | {compression_level, integer()}
                               | {buffer_size, integer()}].
-export_type([cowdb_stream/0]).

%%% Interface functions %%%

%% @doc open a new stream
-spec open(Fd::cowdb_file:cowdb_file()) -> {ok, cowdb_stream()}.
open(Fd) ->
    open(Fd, []).

%% @doc open a new stream
-spec open(Fd::cowdb_file:cowdb_file(), Options::cowdb_stream_options())
    -> {ok, cowdb_stream()}.
open(Fd, Options) ->
    gen_server:start_link(cowdb_stream, {Fd, Options}, []).

%% @doc close the stream
-spec close(Stream::cowdb_stream()) -> ok.
close(Pid) ->
    gen_server:call(Pid, close, infinity).

%% @doc copy a stream from one file to another
-spec copy_to_new_stream(Fd::cowdb_stream(), PosList::[integer()],
                         DestFd::cowdb_stream()) -> ok | {error, term()}.
...
%% fun(Chunk, Acc) -> Acc2
%% '''
-spec foldl(Fd::cowdb_stream(), PosList::[integer()], Fun::fun(), Acc::any()) -> Acc2::any().
foldl(_Fd, [], _Fun, Acc) ->
    Acc;
foldl(Fd, [Pos|Rest], Fun, Acc) ->
    {ok, Bin} = cowdb_file:pread_iolist(Fd, Pos),
    foldl(Fd, Rest, Fun, Fun(Bin, Acc)).

%% @doc like `fold/4' but check the signature.
%%
-spec foldl(Fd::cowdb_stream(), PosList::[integer()], Md5::binary(),
            Fun::fun(), Acc::any()) -> Acc2::any().
foldl(Fd, PosList, <<>>, Fun, Acc) ->
    foldl(Fd, PosList, Fun, Acc);
foldl(Fd, PosList, Md5, Fun, Acc) ->
    foldl(Fd, PosList, Md5, cowdb_util:md5_init(), Fun, Acc).

%% @doc same as fold but decode the chunk if needed.
-spec foldl_decode(Fd::cowdb_stream(), PosList::[integer()], Md5::binary(),
...
foldl_decode(Fd, PosList, Md5, Enc, Fun, Acc) ->
    ...
    Result = foldl_decode(
        DecDataFun, Fd, PosList, Md5, cowdb_util:md5_init(), Fun, Acc
    ),
    DecEndFun(),
    Result.

foldl(_Fd, [], Md5, Md5Acc, _Fun, Acc) ->
    Md5 = cowdb_util:md5_final(Md5Acc),
    Acc;
foldl(Fd, [{Pos, _Size}], Md5, Md5Acc, Fun, Acc) -> % 0110 UPGRADE CODE
    foldl(Fd, [Pos], Md5, Md5Acc, Fun, Acc);
foldl(Fd, [Pos], Md5, Md5Acc, Fun, Acc) ->
    {ok, Bin} = cowdb_file:pread_iolist(Fd, Pos),
    Md5 = cowdb_util:md5_final(cowdb_util:md5_update(Md5Acc, Bin)),
    Fun(Bin, Acc);
foldl(Fd, [{Pos, _Size}|Rest], Md5, Md5Acc, Fun, Acc) ->
    foldl(Fd, [Pos|Rest], Md5, Md5Acc, Fun, Acc);
foldl(Fd, [Pos|Rest], Md5, Md5Acc, Fun, Acc) ->
    {ok, Bin} = cowdb_file:pread_iolist(Fd, Pos),
    foldl(Fd, Rest, Md5, cowdb_util:md5_update(Md5Acc, Bin), Fun, Fun(Bin, Acc)).


%% @doc retrieve all chunks in a range.
-spec range_foldl(Fd::cowdb_stream(), PosList::[integer()],
...
foldl_decode(DecFun, Fd, [Pos], Md5, Md5Acc, Fun, Acc) ->
    {ok, EncBin} = cowdb_file:pread_iolist(Fd, Pos),
    Md5 = cowdb_util:md5_final(cowdb_util:md5_update(Md5Acc, EncBin)),
    Bin = DecFun(EncBin),
    Fun(Bin, Acc);
...
foldl_decode(DecFun, Fd, [Pos|Rest], Md5, Md5Acc, Fun, Acc) ->
    {ok, EncBin} = cowdb_file:pread_iolist(Fd, Pos),
    Bin = DecFun(EncBin),
    Md5Acc2 = cowdb_util:md5_update(Md5Acc, EncBin),
    foldl_decode(DecFun, Fd, Rest, Md5, Md5Acc2, Fun, Fun(Bin, Acc)).

gzip_init(Options) ->
    case cowdb_util:get_value(compression_level, Options, 0) of
...
-spec write(Stream::cowdb_stream(), Bin::binary()) -> ok | {error, term()}.
write(_Pid, <<>>) ->
    ok;
write(Pid, Bin) ->
...
init({Fd, Options}) ->
    ...
    case cowdb_util:get_value(encoding, Options, identity) of
    identity ->
        identity_enc_dec_funs();
    gzip ->
        gzip_init(Options)
    ...
    {ok, #stream{
            fd=Fd,
            md5=cowdb_util:md5_init(),
            identity_md5=cowdb_util:md5_init(),
            encoding_fun=EncodingFun,
            end_encoding_fun=EndEncodingFun,
            max_buffer=cowdb_util:get_value(
...
handle_call({write, Bin}, _From, Stream) ->
    ...
    if BinSize + BufferLen > Max ->
            ...
            IdenMd5_2 = cowdb_util:md5_update(IdenMd5, WriteBin),
            {WrittenLen2, Md5_2, Written2} = case EncodingFun(WriteBin) of
                ...
                WriteBin2 ->
                    {ok, Pos, _} = cowdb_file:append_binary(Fd, WriteBin2),
                    WrittenLen1 = WrittenLen + iolist_size(WriteBin2),
                    Md5_1 = cowdb_util:md5_update(Md5, WriteBin2),
...
handle_call(close, _From, Stream) ->
    ...
    IdenMd5Final = cowdb_util:md5_final(cowdb_util:md5_update(IdenMd5, WriteBin)),
    WriteBin2 = EncodingFun(WriteBin) ++ EndEncodingFun(),
    Md5Final = cowdb_util:md5_final(cowdb_util:md5_update(Md5, WriteBin2)),
