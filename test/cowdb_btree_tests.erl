...
setup_red() ->
    ...
    {Fd, Btree} = setup(),
    {ok, Btree1} = cowdb_btree:add_remove(Btree, EvenOddKVs, []),
    {Fd, Btree1}.
setup_red(_) ->
    setup_red().
...
should_create_zero_sized_btree(_, {_, Btree}) ->
    ?_assertMatch(0, cowdb_btree:size(Btree)).
...
should_fold_over_empty_btree(_, {_, Btree}) ->
    {ok, _, EmptyRes} = cowdb_btree:fold(Btree, fun(_, X) -> {ok, X+1} end, 0),
    ?_assertEqual(EmptyRes, 0).

should_add_all_keys(KeyValues, {Fd, Btree}) ->
    {ok, Btree1} = cowdb_btree:add_remove(Btree, KeyValues, []),
    [
...
should_have_non_zero_size(Btree) ->
    ?_assert(cowdb_btree:size(Btree) > 0).

should_have_lesser_size_than_file(Fd, Btree) ->
    ?_assert((cowdb_btree:size(Btree) =< cowdb_file:bytes(Fd))).

should_keep_root_pointer_to_kp_node(Fd, Btree) ->
    ?_assertMatch({ok, {kp_node, _}},
                  cowdb_file:pread_term(Fd, element(1, Btree#btree.root))).

should_remove_all_keys(KeyValues, Btree) ->
    Keys = keys(KeyValues),
    {ok, Btree1} = cowdb_btree:add_remove(Btree, [], Keys),
    {
...
should_continuously_add_new_kv(KeyValues, {_, Btree}) ->
    {Btree1, _} = lists:foldl(
        fun(KV, {BtAcc, PrevSize}) ->
            ...
            ?assert(cowdb_btree:size(BtAcc2) > PrevSize),
            {BtAcc2, cowdb_btree:size(BtAcc2)}
        end, {Btree, cowdb_btree:size(Btree)}, KeyValues),
    {
...
should_continuously_remove_keys(KeyValues, {_, Btree}) ->
    ...
    {Btree2, _} = lists:foldl(
        fun({K, _}, {BtAcc, PrevSize}) ->
            ...
            ?assert(cowdb_btree:size(BtAcc2) < PrevSize),
            {BtAcc2, cowdb_btree:size(BtAcc2)}
        end, {Btree1, cowdb_btree:size(Btree1)}, KeyValues),
    {
...
should_insert_keys_in_reversed_order(KeyValues, {_, Btree}) ->
    ...
    {Btree1, _} = lists:foldl(
        fun(KV, {BtAcc, PrevSize}) ->
            ...
            ?assert(cowdb_btree:size(BtAcc2) > PrevSize),
            {BtAcc2, cowdb_btree:size(BtAcc2)}
        end, {Btree, cowdb_btree:size(Btree)}, KeyValuesRev),
    should_produce_valid_btree(Btree1, KeyValues).
...
should_be_empty(Btree) ->
    ?_assertEqual(cowdb_btree:size(Btree), 0).
...
fold_reduce(Btree, Opts) ->
    ...
    FoldFun = fun(GroupedKey, Unreduced, Acc) ->
        {ok, [{GroupedKey, cowdb_btree:final_reduce(Btree, Unreduced)} | Acc]}
    end,
    cowdb_btree:fold_reduce(Btree, FoldFun, [],
...
test_key_access(Btree, List) ->
    ...
    {ok, _, {[], Length}} = cowdb_btree:fold(Btree, FoldFun,
                                             {Sorted, 0}, [{dir, rev}]),
    ok.
...
test_lookup_access(Btree, KeyValues) ->
    ...
    lists:foreach(
        fun({Key, Value}) ->
            ...
            {ok, _, true} = cowdb_btree:fold(Btree, FoldFun,
...
test_final_reductions(Btree, KeyValues) ->
    ...
    {ok, _, FoldLRed} = cowdb_btree:fold(Btree, FoldLFun, 0,
                                          [{start_key, LStartKey}]),
    {ok, _, FoldRRed} = cowdb_btree:fold(Btree, FoldRFun, 0,
